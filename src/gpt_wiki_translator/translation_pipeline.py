...
class TranslationPipeline:
    ...
    def process_single_page(self, title: str):
        ...
        # When --force is used, resolve redirects to get the final target page
        target_exists, final_target_title = self.target_mw.page_exists(target_title, resolve_redirects=self.force)

        # Use the final target title if it was redirected and --force is active
        if self.force and final_target_title != target_title:
            logger.info('Target page %s is a redirect to %s. Using final target.', target_title, final_target_title)
